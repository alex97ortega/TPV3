...
bool PlayPG::initGlobos(){
	...
	for (int i = 0; i < numglobos + numglobosA; i++) { // aleatorio punto 2
		...
		objetos.emplace_back(ptsjuego->dame()->createNormalElement());//llamamos a la factoria
	...
	objetos.emplace_back(ptsjuego->dame()->createPrizeElement());//llamamos a la factoria

	objetos.emplace_back(ptsjuego->dame()->createSpecialElement());//llamamos a la factoria
